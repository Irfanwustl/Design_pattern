...
public:
	virtual std::shared_ptr<Event> buildEvent(std::shared_ptr<DisplayableComponent> cal, std::string name, std::tm when, int recurrEvery, int recurrFor) = 0;
	...
	virtual std::shared_ptr<Calendar> buildCalendar_from_restore(std::string name, size_t years, int tm_start_year,int tm_wday) = 0;
