...
shared_ptr<Calendar> IncrementalCalendarBuilder::buildCalendar(string name, size_t years) {
	...
	for (unsigned int i = 0; i < years; ++i) {
		...
		//currentCalendar->addComponent(buildYear(y, currentCalendar)); // this line adds the years recursively
		currentCalendar->children_tm_vector.push_back(y);
	}
	return currentCalendar;
}

std::shared_ptr<Calendar>IncrementalCalendarBuilder::buildCalendar_from_restore(std::string name, size_t years, int tm_start_year, int tm_wday)
{
	currentCalendar = make_shared<Calendar>(name, years);
	// change dateinfo as this is restoring a calendar
	currentCalendar->dateInfo.tm_year = tm_start_year;
	currentCalendar->dateInfo.tm_wday = tm_wday;
	// construct each year in a recursive way, add each year as a child of the calendar
	for (unsigned int i = 0; i < years; ++i) {
		tm y = currentCalendar->dateInfo;
		y.tm_year += i;
...
shared_ptr<Event> IncrementalCalendarBuilder::buildEvent(shared_ptr<DisplayableComponent> cal, string name, tm when, int recurrEvery, int recurrFor) {
	shared_ptr<Event> newly_built_event(new Event(when, cal, name));
	newly_built_event->my_recurr_every = recurrEvery;
	newly_built_event->my_recurr_for = recurrFor;
	return newly_built_event;
}
...
shared_ptr<DisplayableComponent> IncrementalCalendarBuilder::buildMonth(std::tm d, std::shared_ptr<DisplayableComponent> p) {
	...
	for (int i = 0; i < CalendarComponent::days[index]; ++i) { // for each day in the month
		...
		m->children_tm_vector.push_back(d);
...
shared_ptr<DisplayableComponent> IncrementalCalendarBuilder::buildYear(std::tm d, std::shared_ptr<DisplayableComponent> p) {
	...
	for (unsigned int i = 0; i < CalendarComponent::MONTHS; ++i) {
		...
		y->children_tm_vector.push_back(d);
